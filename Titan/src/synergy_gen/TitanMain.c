...
static uint8_t TitanMain_stack[8192] BSP_PLACE_IN_SECTION_V2(".stack.TitanMain") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
void TitanMain_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&TitanMain, (CHAR*) "TitanMain", TitanMain_func, (ULONG) NULL, &TitanMain_stack, 8192, 1, 1,
                            1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
